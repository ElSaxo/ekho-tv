 ...
 * 
 * protected void callback(long delta) { Log.e("EkhoTV",
 * "FooTimer: expired "+delta+" msec."); TimerTest.numCallbacks--; } }
 * 
 * 
 * class TimerTest {
 * 
 ...
 * 
 * if (i != 0) { Log.e("EkhoTV", "*** starting again"); }
 * 
 * for (numCallbacks = 0; numCallbacks < 5; numCallbacks++) { String s =
 * "Gimple " + numCallbacks; Timer t = new FooTimer(s);
 * 
 * int time = rand.nextInt() % 5000; if (time < 0) { time = -time; }
 * 
 * Log.e("EkhoTV", "Starting "+s+" " + time + " msec."); t.start(time);
 * 
 * if (rand.nextInt() < 0) { numCallbacks--; Log.e("EkhoTV", "Stopping " + s);
 * t.stop(); } }
 * 
 * while (numCallbacks > 0) { try { Thread.sleep(1000); } catch
 * (InterruptedException intr) { // do nothing } }
 * 
 * } } }
 */
